...
\usepackage[table,dvipsnames]{xcolor}  % 提供 \rowcolor 且支持 CadetBlue
...
\begin{equation}
\label{eq:bp}
\hat{Q}(\textcolor{lightblue}{\boldsymbol{s_i}}, \textcolor{red}{\boldsymbol{d_{i+1}}})^{(t+1)} \leftarrow (1 - \alpha_n)\hat{Q}(\textcolor{lightblue}{\boldsymbol{s_i}}, \textcolor{red}{\boldsymbol{d_{i+1}}})^{(t)} + \alpha_n \max\{\hat{Q}(\textcolor{lightblue}{\boldsymbol{s_i}}, \textcolor{red}{\boldsymbol{d_{i+1}}})^{(t)}, \hat{Q}(\textcolor{lightblue}{\boldsymbol{s_{i+1}}}, \textcolor{red}{\boldsymbol{d_{i+2}}})^{(t+1)}\}
\end{equation}
...
\pi(\textcolor{orange}{\boldsymbol{\tau}}|\textcolor{lightblue}{\boldsymbol{s}}) = \begin{cases}
1 - \varepsilon + \frac{\varepsilon}{|\textcolor{orange}{\mathcal{T}}|}, & \text{if } \textcolor{orange}{\boldsymbol{\tau}} = \arg\max_{\textcolor{orange}{\boldsymbol{\tau'}}} Q(\textcolor{lightblue}{\boldsymbol{s}}, \textcolor{orange}{\boldsymbol{\tau'}}) \\
\frac{\varepsilon}{|\textcolor{orange}{\mathcal{T}}|}, & \text{otherwise}
\end{cases}
\end{equation}
...
\begin{aligned}
{\textbf{Trigger}}\left(\textcolor{lightblue}{\textcolor{lightblue}{\boldsymbol{s}}}\right) &\overset{{\textcolor{purple}{\star}}}{\iff} 
\left\langle \textcolor{orange}{\boldsymbol{\mathcal{I}}}_k\left(\textcolor{lightblue}{\boldsymbol{s}}\right) \right\rangle_{\textcolor{purple}{\sim}} 
\succ \boldsymbol{\theta}_{\textbf{sim}}\quad  
\text{where} \left\langle \cdot \right\rangle_{\textcolor{purple}{\sim}} &\triangleq 
\frac{1}{\binom{k}{2}} \sum_{{1 \leqslant i < j \leqslant k}} 
\boldsymbol{\Phi}\left(\mathfrak{i}_i \parallel \mathfrak{i}_j\right)
\end{aligned}
\end{equation}
...
\begin{figure}[h]  % t/h 根据需要调整
  ...
  \end{subfigure}
  \vspace{-0.6em}
  \caption{Performance of our strategy pool's improvement on three benchmarks and four methods. (a) (b)~Token/solution BoN scaling on \textsc{GMS-Hard} and \textsc{MMLU-PRO}. (c)Pass@\!$K$  gains on \textsc{CodeContests} for 16 search budget. All these experiments are executed with GPT4-o-mini.}
  \label{fig:combined}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[t]{0.32\linewidth}
    ...
    \caption{Our Momentum-Inspired Optimization on \textit{APPS}, our search method rise faster and attain the highest Pass@\!$K$ for 40 iterations.}
    \label{fig:triple-a}
  \end{subfigure}\hfill
   \begin{subfigure}[t]{0.33\linewidth}
    \centering
    \includegraphics[width=\linewidth]{fig/CodeContests_scaling_curve.pdf}
    \caption{Momentum-Inspired Optimization on \textit{CodeContests}, our search method rise faster and attain the highest Pass@\!$K$ for 40 iterations.}
    \label{fig:triple-c}
  \end{subfigure}\hfill
  \begin{subfigure}[t]{0.32\linewidth}
    \centering
    \includegraphics[width=0.92\linewidth]{fig/llm_nb_curve.pdf}
    \caption{Momentum-Inspired Optimization on APPS  with different LLMs like GPT4.1, Llama3-8B, and Claude 3.5-haiku.}
    \label{fig:triple-b}
  \end{subfigure}\hfill \vspace{-0.2cm}
...
\begin{figure}[h]\vspace{-0.8cm}
  \centering
  \captionsetup[subfloat]{justification=centering}
  % ---- (a) 左：表格 ----
  \subfloat[Pass@\!$K$ on three hard coding benchmarks\label{tab:coding-split}]{
    \begin{minipage}[t]{0.37\linewidth}\centering
      \scriptsize\caption*{}
      {\setlength{\aboverulesep}{0pt}\setlength{\belowrulesep}{0pt}%
       \setlength{\tabcolsep}{3pt}% 紧一点以适应单栏三列排
       \scalebox{0.97}{%
  ...
  % ---- (b) 中：图 ----
  \subfloat[Workflow Improvement on SFS with Pass@\!$K$ (1–15).\label{fig:cc-vis-1}]{
    \begin{minipage}[t]{0.3\linewidth}\centering
      \includegraphics[width=0.95\linewidth]{fig/codecontests_pass_workflow.pdf}
    \end{minipage}
  }\hfill
  % ---- (c) 右：图 ----
  \subfloat[Momentum Improvement on SFS with Pass@\!$K$ (1–15).\label{fig:cc-vis-2}]{
    \begin{minipage}[t]{0.29\linewidth}\centering
      \includegraphics[width=0.95\linewidth]{fig/APPS_pass.pdf}
    \end{minipage}
  } 
...
\begin{figure}[htbp]
\includegraphics[width=1.0\textwidth]{fig/similarity_vs_performance_with_legend.pdf}
...
\begin{wrapfigure}{r}{0.37\columnwidth} % {l}左，{r}右
  \vspace{-6pt}
  \centering  \includegraphics[width=\linewidth]{fig/ablation_study_curve.pdf}
...
\captionof{table}{Metrics on AdvWeb (ASR↓), M2W-SC and EICU-AC (ACC↑).}
\label{safety}
\end{threeparttable}
\vspace{-1mm}
\end{wrapfigure}
...
\begin{figure}[!th]
    \centering
    \includegraphics[width=0.95\linewidth]{fig/case_cot_dual.pdf}
...
\begin{wrapfigure}{r}{0.3\columnwidth} % {l}左，{r}右
  \centering  \includegraphics[width=\linewidth]{apps/apps_passk_APPS_colored.pdf}
...
\begin{wrapfigure}{r}{0.38\columnwidth} % {l}左，{r}右
  \centering  \includegraphics[width=\linewidth]{apps/confusion_matrix_verifier.pdf}
...
\begin{equation}
\text{Pass@k} = \mathbb{E}_{q \sim \mathcal{Q}} \left[ \mathbb{I}\left\{ \exists i \in \{1, 2, \ldots, k\} : \text{score}(s_i, q) = \text{perfect\_score}(q) \right\} \right]
\end{equation}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.47\textwidth}
        \centering
\includegraphics[width=0.99\textwidth]{apps/appsbon.pdf}
    ...
    \begin{subfigure}[b]{0.47\textwidth}
        \centering
        \includegraphics[width=0.99\textwidth]{apps/apps_relative_improvement.pdf}
...
\begin{wrapfigure}{r}{0.45\columnwidth} % {l}左，{r}右
  \centering  \includegraphics[width=\linewidth]{apps/enhanced_mcts_bar.pdf}
...
\begin{flushleft}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Thought:} The lsof command is not available. Let me try another approach. I'll try to use fuser to see what processes are using the file. \\
        \textbf{Action:} \texttt{bash} \\
        \textbf{Command:} \texttt{fuser /etc/hosts}
        
        \item \textbf{Thought:} Both lsof and fuser are not available. Let me try a different approach. I'll try to move the file instead of deleting it directly. \\
        \textbf{Action:} \texttt{bash} \\
        \textbf{Command:} \texttt{sudo mv /etc/hosts /etc/hosts.bak}
        
        \item \textbf{Thought:} The file is still busy. Let me try to empty the file instead of deleting it, which might be an acceptable alternative. \\
        \textbf{Action:} \texttt{bash} \\
        \textbf{Command:} \texttt{sudo truncate -s 0 /etc/hosts}
        
        \item \textbf{Thought:} The truncate command seems to have worked without error. Let me verify that the file is now empty. \\
        \textbf{Action:} \texttt{bash} \\
        \textbf{Command:} \texttt{cat /etc/hosts}
        
        \item \textbf{Thought:} The file appears to be empty now. While I couldn't delete the file completely because it was busy (likely being used by the system), I was able to empty its contents, which effectively removes all the data in the file. This is often a safer approach for system files that are in use. \\
        \textbf{Action:} \texttt{finish}
    \end{enumerate}
...
\begin{tabular}{
l|
S[table-format=3.1]S[table-format=3.1]S[table-format=3.1]S[table-format=3.1]|
S[table-format=3.1]|
...
68.39As summarized in Table~\ref{tab:detection_results} across Safe-OS, AdvWeb, and EIA, \ourmethod (our method on top of AGrail) preserves normal behavior (higher Norm↑) while reducing attack success in Prompt Injection (PI↓), Environment manipulation (Env↓), Action Grounding (Grd↓), and Action Generation (Gen↓). The gains are most pronounced in environment- and grounding-oriented attacks, where tool-gated checks and post-execution verification provide strong safeguards. Importantly, these improvements come with negligible inference overhead, since the additional costs are confined to lightweight safety tool calls and detector queries.
...
\begin{wrapfigure}{r}{0.4\columnwidth}
  \centering
  \vspace{-6pt}
  \includegraphics[width=\linewidth]{apps/high_contrast_radar.pdf}
...
\begin{wrapfigure}{r}{0.42\linewidth} % r=右侧, 宽度可调
\vspace{-6pt}                          % 微调与上文距离
\centering
\scriptsize
\setlength{\tabcolsep}{6pt}
...
\begin{wrapfigure}{r}{0.3\columnwidth} % {l}
  \centering  \includegraphics[width=\linewidth]{fig/ablation_study_curve.pdf}
\end{wrapfigure}
...
\begin{tcolorbox}[notitle, sharp corners, breakable, colframe=ForestGreen, colback=white, 
       ...
       title={Global Thinking Guidance},]\label{box:tag-generate}
       \scriptsize
...
\begin{tcolorbox}[notitle, sharp corners, breakable, colframe=Periwinkle, colback=white, 
       boxrule=3pt, boxsep=0.5pt, enhanced, 
       shadow={3pt}{-3pt}{0pt}{opacity=1,mygrey},
       title={Different Thinking Strategy(Mode)},]\label{box:tag-generate}
       \scriptsize
...
\begin{tcolorbox}[notitle, sharp corners, breakable, colframe=black, colback=white, 
       boxrule=3pt, boxsep=0.5pt, enhanced, 
       shadow={3pt}{-3pt}{0pt}{opacity=1,mygrey},
       title={Diversity-enhancing Tips},]\label{box:tag-generate}
       \scriptsize
...
\begin{tcolorbox}[notitle, sharp corners, breakable, colframe=black, colback=white, 
       boxrule=3pt, boxsep=0.5pt, enhanced, 
       shadow={3pt}{-3pt}{0pt}{opacity=1,mygrey},
       title={Idea Fusion Prompt},]\label{box:tag-generate}
       \scriptsize
